...
Launch an orchestrated review-and-fix loop for an existing diff. Treat the invoking agent as a parent launcher: it should start a dedicated orchestrator subagent, pass the current diff and constraints to it, and let that subagent manage reviewer and implementer workers.
...
3. Start an orchestrator subagent and give it:
...
- `references/roles.md`: responsibilities and guardrails for each subagent type
- `references/contracts.md`: fixed Markdown message formats to pass between subagents
